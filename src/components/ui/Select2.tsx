...
const Select2 = () => {
    ...
    const [selectedValue, setSelectedValue] = useState("Type (Boat Tours)")

    // array of options 

    const options = ['Adeventure','Beaches','Boat Tours','City Tours','Honeymoon','Mesuem Tours']
        ...
        <div>
            ...
            <div className={`${isOpen ? 'visible top-0 opacity-100' : 'invisible -top-4 opacity-0'} relative overflow-y-auto max-h-44 my-4 w-72 rounded-xl py-4 border duration-300`}>
                ...
                {options?.map((option, idx) => (
                    ...
                    <div key={idx} onClick={(e) => { setSelectedValue(e.target.textContent); setIsOpen(false); }} className="px-6 py-2 text-gray-500 cursor-pointer hover:text-orange-400">
